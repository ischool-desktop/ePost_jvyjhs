        ...
        static void Program_Click(object sender_, EventArgs e_)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                for (int subjectIndex = 1; subjectIndex <= conf.SubjectLimit; subjectIndex++)
                {
                    ...
                    table.Columns.Add("學分" + subjectIndex);
                    ...
                    table.Columns.Add("上學期學分" + subjectIndex);
               ...
               // foreach (string name in ATTypeList)
                ...
                bkw.DoWork += delegate(object sender, System.ComponentModel.DoWorkEventArgs e)
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            XElement elm = XElement.Parse(conf.Memo);
                            if (elm != null)
                            {
                                if (elm.Element("補考日期") != null)
                                    DateStr = elm.Element("補考日期").Value;

                                if (elm.Element("備註") != null)
                                    MemoStr = elm.Element("備註").Value;
                            }
                        }
                        catch (Exception ex) { }
...
//                                    data["學分" + subjectIndex] = dr["學分數" + subjectIndex];
                                    ...
                                    // 處理科目學分數
                                    string ScoreStr = dr["學分數" + subjectIndex] + "=";

                                    // 未取得學分epost成績加*
                                    if (dr["科目取得學分" + subjectIndex].ToString() == "否")
                                    {
                                    //    data["成績" + subjectIndex] = "*" + dr["學期科目成績" + subjectIndex];
                                        ScoreStr += "0=" + dr["科目必選修" + subjectIndex].ToString().Replace("修", "");
                                    }
                                        
                                    else
                                    {
                                      //  data["成績" + subjectIndex] = dr["學期科目成績" + subjectIndex];
                                        ScoreStr +=dr["學分數" + subjectIndex]+"="+ dr["科目必選修" + subjectIndex].ToString().Replace("修", "");
                                    }

                                    data["學分" + subjectIndex]=dr["學分" + subjectIndex] = ScoreStr;
                        ...
                        #region 產生對照欄位
                        StreamWriter sw = new StreamWriter(Application.StartupPath + "\\table.txt", false);
                        foreach (DataColumn dc in table.Columns)
                            sw.WriteLine(dc.ColumnName);
                        sw.Flush();
                        sw.Close();

                        StreamWriter swe = new StreamWriter(Application.StartupPath + "\\eposttable.txt", false);
                        foreach (DataColumn dc in  _dtEpost.Columns)
                            swe.WriteLine(dc.ColumnName);
                        swe.Flush();
                        swe.Close();
